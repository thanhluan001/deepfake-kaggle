...
class FaceExtractor:
    ...
    def process_videos(self, input_dir, filenames, video_idxs, margin=0.2):
        ...
        for v in range(len(tiles)):
            ...
            for i in range(len(detections)):
                ...
                faces = self._add_margin_to_detections(detections[i], frame_size, margin)
    ...
    def process_video(self, video_path, margin=0.2):
        """Convenience method for doing face extraction on a single video
        with some margin. :D 
        """
        input_dir = os.path.dirname(video_path)
        filenames = [ os.path.basename(video_path) ]
        return self.process_videos(input_dir, filenames, [0], margin)
